...
const ApiMenu = ({
  ...
  const openRouterEndpoint = useStore((state) => state.openRouterEndpoint);
  const setOpenRouterEndpoint = useStore((state) => state.setOpenRouterEndpoint);
  ...
  const [_openRouterEndpoint, _setOpenRouterEndpoint] = useState<string>(openRouterEndpoint);

  const handleSave = () => {
    setApiKey(_apiKey);
    setOpenRouterApiKey(_openRouterApiKey);
    setApiEndpoint(_apiEndpoint);
    setOpenRouterEndpoint(_openRouterEndpoint);
  ...
  return (
    ...
    >
      <div className='p-6 border-b border-gray-200 dark:border-gray-600'>
        ...
        <div className='flex gap-2 items-center justify-center mt-2'>
          <div className='min-w-fit text-gray-900 dark:text-gray-300 text-sm'>
            {t('openRouterEndpoint.inputLabel', { ns: 'api' })}
          </div>
          <input
            type='text'
            className='text-gray-800 dark:text-white p-3 text-sm border-none bg-gray-200 dark:bg-gray-600 rounded-md m-0 w-full mr-0 h-8 focus:outline-none'
            value={_openRouterEndpoint}
            onChange={(e) => {
              _setOpenRouterEndpoint(e.target.value);
            }}
          />
        </div>
